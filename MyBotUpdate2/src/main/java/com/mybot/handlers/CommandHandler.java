...
public class CommandHandler {
    ...
    public CommandHandler() {
        this.weatherAPI = createWeatherAPI();
        this.locationService = new LocationService();  // Конструктор без параметров
        this.locationService.setWeatherAPI(weatherAPI);  // Устанавливаем WeatherAPI отдельно
    }
    
    // Protected метод для переопределения в тестах
    ...
    public void handleCommand(String command, Long chatId, String userName, TelegramLongPollingBot bot) {
        ...
        switch (baseCommand) {
            ...
            case "/about":
                message.setText("🤖 *Погодный Бот v2.0*\n\n" +
                               "*✨ Новые возможности:*\n" +
            ...
            case "/authors":
                message.setText("👨‍💻 *Авторы проекта v2.0:*\n\n" +
                               "• Ушков Роман\n" +
                               "• Крылосов Даниил\n\n" +
                               "Теперь с кэшированием и геолокацией!");
                message.setParseMode("Markdown");
                break;
