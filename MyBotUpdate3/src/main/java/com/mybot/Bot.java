...
public class Bot extends TelegramLongPollingBot {
    ...
    private void handleInlineQuery(InlineQuery inlineQuery) {
        ...
        try {
            // БЫСТРЫЙ ОТВЕТ - сначала отправляем пустой результат
            List<InlineQueryResult> results = new ArrayList<>();
            
            // Сразу отправляем ответ, чтобы Telegram не ждал
            ...
            // Теперь в фоне получаем погоду и отправляем обновленный результат
            new Thread(() -> {
                try {
                    // Получаем погоду
                    ...
                    finalAnswer.setCacheTime(300); // Кэшируем на 5 минут
                    ...
                    try {
                        sendInlineError(queryId, city, "Не удалось получить погоду");
